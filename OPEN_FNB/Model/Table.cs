...
namespace OPEN_FNB.Model
{
    internal class Table
    {
        ...
        public static List<Table> getAllWithCondition(MySqlConnection conn,string condition)
        {
            ...
            try
            {
                MySqlCommand cmd = new MySqlCommand($"select t.id,t.name,t.info,tt.id as typeid, tt.name as typename,b.id as bill from tables t join table_type tt on t.table_type = tt.id left join (select id,table_id from bill where complete=false ) b on b.table_id = t.id where t.name like '%%'" + condition, conn);
                ...
                while (rs.Read())
                {
                    tables.Add(new Table(rs.GetInt16(0), rs.GetString(1), rs.GetString(2), rs.GetInt16(3), rs.GetString(4), rs.IsDBNull(5) ? -1 : rs.GetInt32(5)));
                }
                rs.Close();
            }
            catch (Exception e)
